...
class ThreeUnsharedGenerator(nn.Module):
    
    def __init__(self, batch_size, num_generators, n_z):
        # self.num_gpu = num_gpu
        self.b_size = batch_size//num_generators
        super(ThreeUnsharedGenerator, self).__init__()
        
        self.fcA = nn.Linear(in_features = n_z, out_features = 4*4*64*G_channel_factor, bias=False)
        self.fcB = nn.Linear(in_features = n_z, out_features = 4*4*64*G_channel_factor, bias=False)
        self.fcC = nn.Linear(in_features = n_z, out_features = 4*4*64*G_channel_factor, bias=False)
    ...
    def forward(self, x):#the address of the generator i.e 0, 1 or 2 in this case
        return torch.cat([self.headA(self.fcA(x).view(self.b_size, 64, 4,4)), self.headB(self.fcB(x).view(self.b_size, 64, 4,4)), self.headC(self.fcC(x).view(self.b_size, 64, 4,4))])
